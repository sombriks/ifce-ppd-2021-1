...
public class MessageProcessor {
	...
	public String makeMoveMessage(String move) {
		return "messageType:" + MessageType.MOVE_SEND + ",message:" + move + ",player:" + player + ",\n";
	}
	...
	public void processMessage(String message) {
		...
		case TEXT:
			...
			break;
		case MOVE_SEND:
			Move square = extractMove(message);
			game.take(square);
			break;
		}
	}

	private Move extractMove(String message) {
		String move = message.split(",")[1].split(":")[1];
		String player = message.split(",")[2].split(":")[1];
		return new Move(move, Player.valueOf(player));
	}
